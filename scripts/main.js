...
function init() {
  ...
  // Edit and Delete buttons on records 
  ...
  document.getElementById("search-pattern").addEventListener("input", () => {
    ...
    const { results, error } = performSearch(
      state.transactions,
      pattern,
      isCaseInsensitive
    );
  ...
  document.getElementById("save-settings-btn").addEventListener("click", () => {
    ...
    state.settings.spendingCap =
      parseFloat(document.getElementById("spending-cap").value) || 0;
