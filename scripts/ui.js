...
export function renderDashboard() {
    ...
    // ... (rest of the function is the same, as it relies on formatCurrency)
    ...
    const topCategory = Object.keys(categoryTotals).length ? Object.entries(categoryTotals).sort((a,b) => b[1]-a[1])[0][0] : '-';
...
function renderBudgetStatus(totalSpent) {
    ...
    if (percentage >= 100) {
        alertClass = 'danger';
        message = `<strong>⚠️ Budget Exceeded!</strong> You've spent ${formatCurrency(totalSpent)} of your ${formatCurrency(spendingCap)} budget.`;
        alertMessage = `Budget exceeded by ${formatCurrency(Math.abs(remaining))}.`;
    } else if (percentage >= 80) {
        alertClass = 'warning';
        message = `<strong>⚡ Approaching Limit!</strong> ${formatCurrency(remaining)} remaining.`;
        statusMessage = `Approaching budget limit. ${formatCurrency(remaining)} remaining.`;
    } else {
        alertClass = 'success';
        message = `<strong>✅ On Track.</strong> ${formatCurrency(remaining)} remaining.`;
    }
...
function renderRecentTransactions() {
    ...
    container.innerHTML = `<table><thead><tr><th>Date</th><th>Description</th><th>Amount</th></tr></thead><tbody>
        ${recent.map(t => `<tr><td>${t.date}</td><td>${escapeHtml(t.description)}</td><td>${formatCurrency(t.amount)}</td></tr>`).join('')}
    </tbody></table>`;
}
...
export function renderRecords() {
    ...
    tbody.innerHTML = state.transactions.map(t => `
        <tr data-id="${t.id}">
            <td>${t.date}</td><td>${escapeHtml(t.description)}</td><td>${escapeHtml(t.category)}</td><td>${formatCurrency(t.amount)}</td>
            <td class="actions"><button class="edit-btn">Edit</button><button class="delete-btn danger">Delete</button></td>
        </tr>`).join('');
    cardsDiv.innerHTML = state.transactions.map(t => `
        <div class="transaction-card" data-id="${t.id}">
            <div class="transaction-card-header"><div class="transaction-card-title">${escapeHtml(t.description)}</div><div class="transaction-card-amount">${formatCurrency(t.amount)}</div></div>
            <div class="transaction-card-details">${t.date} • ${escapeHtml(t.category)}</div>
...
export function renderSettings() {
    ...
    for (const currencyCode in rates) {
        ...
        const inputElement = document.getElementById(`rate-${currencyCode.toLowerCase()}`);
        if (inputElement) {
    ...
    categoriesList.innerHTML = categories.map(cat => `
        <div style="display:flex; justify-content:space-between; align-items:center; padding:0.5rem; border:1px solid var(--border); border-radius:4px; margin-bottom:0.5rem;">
            ...
            <button class="remove-category-btn danger" data-category="${escapeHtml(cat)}">Remove</button>
        </div>
    `).join('');
...
export function renderSearchResults(results, error) {
    ...
    resultsDiv.innerHTML = `<p><strong>${results.length}</strong> match(es) found</p><table><thead><tr><th>Date</th><th>Description</th><th>Amount</th></tr></thead><tbody>
        ${results.map(t => `<tr><td>${t.date}</td><td>${t.highlightedDesc}</td><td>${formatCurrency(t.amount)}</td></tr>`).join('')}
    </tbody></table>`;
}
